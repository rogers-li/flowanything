...
func Build(field *Field, sourceMap map[string]interface{}) (interface{}, error) {
	if field == nil {
		return nil, nil
	}
	...
	return field.GetValue(sourceMap)
}

// ParseStructConfig 解析配置的字段映射json字符串
func ParseStructConfig(structConfig string) (*Field, error) {
...
func parseStruct(fieldName string, fieldConfig interface{}) (*Field, error) {
	if utils.IsKind(fieldConfig, reflect.Map) {
		objectMap, _ := fieldConfig.(map[string]interface{})
		// 如果配置的是map，则解析为object
		resultField := NewField(fieldName, FieldObject)
